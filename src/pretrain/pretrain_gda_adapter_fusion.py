...
wandb.init(project="july_pretrain_disgene_gda_fusion")
...
def parse_config():
    ...
    parser.add_argument("--prot_model_step", type=int, default=6)
    parser.add_argument("--disease_model_step", type=int, default=6)
    ...
    parser.add_argument(
        "--save_path_prefix",
        type=str,
        default="../../save_model_ckp/pretrain/gda_fusion/",
...
def train(args):
    ...
    prot_model_path = os.path.join(
        args.prot_model_path, f"prot_adapter_step_{args.prot_model_step}"
    )
    disease_model_path = os.path.join(
        args.disease_model_path, f"disease_adapter_step_{args.disease_model_step}"
    )
    model.init_adapters()
    # model.load_adapters(prot_model_path, disease_model_path)
    # model.add_classification_head(out_dim=2)
    # if args.use_stack:
    #     model.add_stack_gda(args.reduction_factor)
