...
from src.utils.tdc_ppi_processor import TDCPPIProcessor

import wandb

wandb.init(project="july_finetune_PPI_cls_with_adapter")
...
def train_an_epoch(model, train_dataloader, optimizer, loss, args):
    ...
    for step, batch in enumerate(train_dataloader):
        prot_inputs1, prot_inputs2, label_inputs = (
            batch[0].to(args.device),
            batch[1].to(args.device),
            batch[2].to(args.device),
        )
        optimizer.zero_grad()
        x = model.predict(prot_inputs1, prot_inputs2)
        x = model.dropout(x)
        logits = model.cls(x)
...
def evaluate(model, test_dataloader, args):
    ...
    with torch.no_grad():
        for step, batch in enumerate(test_dataloader):
            prot_inputs1, prot_inputs2, label_ids = batch
            prot_inputs1, prot_inputs2, label_ids = (
                prot_inputs1.to(args.device),
                prot_inputs2.to(args.device),
                label_ids.to(args.device),
            )
            if (
                args.device == "cuda"
                and args.n_gpu > 1
                and prot_inputs1.size()[0] % args.n_gpu != 0
            ):  # For fixing the bug of sample size less than the gpu size.
                print(
                    "input_ids.size()[0] < self.args.n_gpu ---> back to single gpu mode"
                )
                x = model.module_predict(prot_inputs1, prot_inputs2)
                logits = model.cls(x)
            else:
                x = model.predict(prot_inputs1, prot_inputs2)
                logits = model.cls(x)
...
def train(args):
    ...
    if args.model_path:
        ...
        if args.use_adapter:
            model.load_prot_adapter(prot_model_path)
    ...
    if args.device == "cuda" and args.n_gpu > 1:  # use multiple gpu
        model.prot_encoder = torch.nn.DataParallel(model.prot_encoder)
