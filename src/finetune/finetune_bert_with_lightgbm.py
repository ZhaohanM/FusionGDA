...
wandb.init(project="july_finetune_model_w_lightgbm")
...
def train(args):
    # defining parameters
    args.model_short = (
        args.model_path.split("/")[-2] + "_" + args.model_path.split("/")[-1]
    )
    print(f"model name {args.model_short}")
    input_feat_file = os.path.join(
        args.input_feature_save_path, f"{args.model_short}_input_feat_{args.test}.npz"
    )
    if os.path.exists(input_feat_file):
        ...
        if args.model_path:
            ...
            prot_model = BertModel.from_pretrained(args.prot_encoder_path).to(
                args.device
            )
